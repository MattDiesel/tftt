...
\section{Method} % (fold)
    ...
    \subsection{Distribution} % (fold)
        ...
        \begin{algorithm}[!htbp]
            ...
            \begin{algorithmic}
                ...
                \Ensure A complete ghost set $\mathbb{G}_{\rho,b}$, for every pair of processes $\rho$ and $b$
                \Statex
                \State $\mathbb{G} \gets \emptyset$
                \ForEach {leaf $\mathcal{C} \in \mathbb{L}$}
                    \State $\mathcal{C}\acc\{\vect{\mathcal{P}}, \vect{\beta},\alpha\}\gets \textproc{CalcPoisCoefs}(\mathcal{C}$
                    \Statex
                    \ForEach {poisson neighbour $\mathcal{P} \in \mathcal{C}\acc\vect{\mathcal{P}}$}
                        \If {$\mathcal{P}\acc\textproc{rank} \neq \mathcal{C}\acc\textproc{rank}$}
                            \State $\mathbb{G}_{\mathcal{C}\acc\textproc{rank},\mathcal{P}\acc\textproc{rank}} \gets \mathcal{P}$
                        \EndIf
                    \EndFor
                \EndFor
            \end{algorithmic}
        ...
        \clearpage
        There is now sufficient information in a process' own cell and ghost sets to generate the border set using the method in Algorithm~\ref{alg:borders-gen}.
    ...
    \subsection{Ghost Synchronisation} % (fold)
        ...
        With each iteration, the data for each process' ghost cells becomes out of date. At the beginning of every iteration, synchronisation occurs to renew this data. 

        The set up described previously in Section~\ref{sec:ghostsandborders} is designed to minimise this process. By making use of the equivalence of the ghost and border sets for two processes, the only information transfer required by Algorithm~\ref{alg:sync-sendrecv} is the data from the border cells. Since it is required once per iteration, ghost synchronisation is by far the most common parallel operation required and so this optimisation is an important one.  
        ...
        \begin{algorithm}[!htbp]
            ...
            \begin{algorithmic}
                \Require {Ghost cell sets $\mathbb{G}$ that are equivalent to the border set $\mathbb{B}$ on the current process}
                \Statex
    ...
    \subsection{Rebalancing} % (fold)
        ...
        \subsubsection{Update Cell Ranks} % (fold)
            ...
            \begin{algorithm}[!htbp]
                ...
                \begin{algorithmic}
                    ...
                    \ForEach {cell $\mathcal{C} \in \{\text{the first }\Delta_l\text{ cells in }\mathbb{L}\}$}
                        \Comment{For right, $\mathcal{C} \in \{\text{the last }\Delta_r\text{ cells in }\mathbb{L}\}$}
        ...
        \subsubsection{Generate Send Sets} % (fold)
            ...
            The two sets $T_l$ and $T_r$ resulting from Algorithm~\ref{alg:rebalance-gensendsets} and its sending right equivalent are then transmitted in Algorithm~\ref{alg:rebalance-sendsets}. The corresponding sets on the foreign process $U_l$ and $U_r$ receive these values.
            ...
            The combined set $\vect{U}=\vect{U}_l\cup\vect{U}_r$ now contains all the additional information required by the process that was not available prior, and is needed to fully describe the new leaf, ghost and border sets. 
        ...
        \subsubsection{Insert Cell Set} % (fold)
            ...
            Each process inserts the received cells in $\vect{U}$ and calculates the poisson neighbourhood. All the new cells must be inserted before calculating poisson neighbourhoods, as the correct neighbourhood may include new cells that did not exist prior.
