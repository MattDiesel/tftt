...
void plotAdaptVectors()
{
    std::ofstream ofs("adaptvect.dat");

    int ad;
    for (int n = 0; n < 2; n++) {
        for (auto gh : tftt::gtree.rawGhosts[n]) {
            for (int d = 0; d < 4; d++) {
...
void plotAdaptHoldVectors()
{
    std::ofstream ofs("holdvect.dat");

    int ad;
    for (auto ghb : tftt::gtree.rawGhosts) {
        for (auto gh : ghb) {
            for (int d = 0; d < 4; d++) {
...
int main(int argc, char* argv[])
{
    ...
    if (world.rank() == 1) {
        double p2[2] = {0.49, 0.05};
        tftt::cell_t check = tftt::atPos(p2);

        double p3[2] = {0.54, 0.05};
        tftt::cell_t check2 = tftt::atPos(p3);

        tftt::drawPoissonNeighbourhood("test.dat", check);

        std::cout << "Rank: " << (int)check2.rank() << "\n";
    }

    // What happens if we refine the cell at p
    double p[2] = {0.42, 0.11};
    tftt::cell_t target = tftt::atPos(p);

    tftt::adaptParSwBegin();

    tftt::adaptParSwSetRefine(target);

    tftt::adaptSwCommit();

    // plotAdaptVectors();
    // plotAdaptHoldVectors();

    std::cout << "Reached this bit\n";
